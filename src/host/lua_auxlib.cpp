...
LUALIB_API int luaL_loadfile (lua_State* L, const char* filename)
{
	...
	if (filename[0] == '$') {
		z = build_load_embedded_script(L, filename + 2); /* Skip over leading "$/" */
		if (z != OKAY) return z;
	}
	...
	if (z != OKAY) {
		...
		if (script_dir && script_dir[0] == '$') {
			...
			z = build_load_embedded_script(L, test_name + 2); /* Skip over leading "$/" */
	...
	if (z != OKAY) {
		z = build_load_embedded_script(L, filename);
	}
...
static int chunk_wrapper(lua_State* L)
{
	...
	ptr = const_cast<char* >( strrchr(filename, '/') );
	if (ptr) *ptr = '\0';
	lua_pushstring(L, filename);
