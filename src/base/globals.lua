...
---
	...
	function include(fname)
		local fullPath = build.findProjectScript(fname)
...
---
	...
	-- build.override(_G, "require", function(base, modname, versions)
	-- 	local result, mod = pcall(base,modname)
	-- 	if not result then
	-- 		if (build.downloadModule(modname, versions)) then
	...
	-- 		end
	-- 	end
	-- 	if mod and versions and not build.checkVersion(mod._VERSION, versions) then
