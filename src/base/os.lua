...
--
	...
	build.override(os, "execute", function(base, cmd)
...
--
	...
	function os.get()
		build.warnOnce("os.get", "os.get() is deprecated, use 'os.target()' or 'os.host()'.")
		return os.target()
	end
	...
	_G_metatable = {
		__index = function(t, k)
			if (k == '_OS') then
				build.warnOnce("_OS+get", "_OS is deprecated, use '_TARGET_OS'.")
		...
		__newindex = function(t, k, v)
			if (k == '_OS') then
				build.warnOnce("_OS+set", "_OS is deprecated, use '_TARGET_OS'.")
...
--
	...
	function os.is(id)
		build.warnOnce("os.is", "os.is() is deprecated, use 'os.istarget()' or 'os.ishost()'.")
...
---
	...
	build.override(os, "isdir", function(base, p)
		p = path.normalize(p)
		return base(p)
	end)
...
---
	...
	build.override(os, "isfile", function(base, p)
		p = path.normalize(p)
		return base(p)
	end)
...
---
	...
	build.override(os, "stat", function(base, p)
		p = path.normalize(p)
		return base(p)
	end)
	...
	function os.uuid(name)
		...
		if name then
			if os._uuids[id] and os._uuids[id] ~= name then
				build.warnOnce(id, "UUID clash between %s and %s", os._uuids[id], name)
			end
			os._uuids[id] = name
		end
		return id
