...
---
	...
	build = build or {}
	build._VERSION = _BUILd_VERSION
	package.loaded["build"] = build

	build.modules = {}
	build.extensions = build.modules
...
---
	...
	function b.alias(scope, canonical, alias)
		scope, canonical = b.resolveAlias(scope, canonical)
...
---
	...
	function build.callArray(funcs, ...)
	...
	function build.callarray(namespace, array, ...)
		local n = #array
		for i = 1, n do
...
---
	...
	function b.checkVersion(version, checks)
		if not version then
			return false
		end
...
--
	...
	function build.findProjectScript(fname)
...
---
	...
	function build.override(scope, name, repl)
		scope, name = b.resolveAlias(scope, name)
		...
		-- Functions from build.main are special in that they are fetched
		...
		if scope == build.main then
			table.replace(build.main.elements, original, scope[name])
		end
	end
...
---
	...
	function b.resolveAlias(scope, name)
...
--
	...
	function build.warnOnce(key, message, ...)
